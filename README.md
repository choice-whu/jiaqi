...
画像只保存在本地 `SharedPreferences`，不会上传自定义服务器。画像会抽取地点类型特征，例如咖啡、书店、电玩、手作、江边、公园、livehouse、商场小店等，并使用正负反馈权重、最近反馈加权、旧反馈衰减、当前时间段加权、同名/近似地点短期去重。

## 推荐链路

```mermaid
flowchart TD
    Tap["用户点击生成新建议"] --> Env["DecisionEnvironmentCache\n极速读取时间/定位/天气"]
    Env --> Topic["RecommendationTopicProvider\n按画像和时间挑主题"]
    Topic --> AI["AiRepository + AiChatClient\nAI 生成候选"]
    AI --> Clean["AiJsonSanitizer\n清洗 JSON/Markdown"]
    Clean --> Resolve["PlaceResolver\n高德 POI 解析"]
    Resolve --> Policy["DecisionPlacePolicy\n距离/营业/地点策略"]
    Policy --> Score["DecisionCandidateScorer\n天气/时间/画像评分"]
    Score --> Card["DecisionCardUiModel\n刮刮乐结果卡"]
    Card --> Pool["DecisionReadyPool\n空闲预取缓存"]
```
...
flowchart TD
    ...
    Decision --> Scorer["DecisionCandidateScorer"]
    Decision --> Policy["DecisionPlacePolicy"]
...
│   ├── decision                            # AI 决策引擎、策略和候选评分
│   ├── environment                         # 时间、位置、天气上下文
│   ├── place                               # 推荐地点解析
│   ├── route                               # 高德路线、POI、兜底估算
│   └── recommendation                      # 本地个性化画像和主题池
└── ui
    ├── wish                                # 心愿池界面
    ├── decision                            # 刮刮乐决策界面、预取池、环境缓存
    ├── timeline                            # 行程/导航相关界面
    ├── components                          # 通用按压反馈
    └── theme                               # 温润极简主题
```
...
& "$env:LOCALAPPDATA\Android\Sdk\platform-tools\adb.exe" shell monkey -p com.example.dateapp -c android.intent.category.LAUNCHER 1
```
...
- `AiChatClient`：AI 请求、响应耗时、JSON 返回。
- `RoutePlanningRepository`：目的地解析、路线规划、兜底估算。
- `DecisionContext`：定位、天气、环境快照。

示例信号：

```text
decision source=PREFERENCE_PROFILE category=play hour=21 ...
decision source=ENGINE_PERSONAL_SCORE targetCategory=play ...
decision source=ENGINE_SELECTED attempt=1 mode=AI_VERIFIED_RESCUE ...
decision source=READY_POOL_HIT targetCategory=play ...
```
...
- 启动：成功
- 崩溃：未发现 `FATAL EXCEPTION` / `AndroidRuntime`
- 环境链路：高德定位与天气接口可返回有效上下文
- AI 链路：`deepseek-v4-flash` 可返回推荐，失败候选会进入高德校验/救援链路

## 后续方向

- 继续拆分 `DecisionViewModel` 的卡片组装和兜底文案，让 ViewModel 更像纯状态机。
- 增加单元测试，覆盖 AI JSON 清洗、地点去重、画像打分、ReadyPool 淘汰策略。
